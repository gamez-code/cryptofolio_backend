...
from coinapi_rest_v1.coinapi_rest_v1 import CoinAPIv1
import datetime, sys

if __name__ == '__main__':

from coinapi_rest_v1.coinapi_rest_v1 import CoinAPIv1
test_key = "45A83179-F3CC-4993-AD28-AD474A487F08"
...
for exchange in exchanges:
    CryptoCurrency.objects.create(dict([('website', exchange['website']), ('exchange_id', exchange['exchange_id']), ('name', exchange['name'])))
...
if __name__ == '__main__':
    from blockchain.exchangerates import get_ticker
    daily_currencies = get_ticker()
    list(map(lambda item: CurrencyModel.objects.create(name=item[0], **item[1].__dict__), daily_currencies.items()))
    get_ticker()
    ...
    for cu in cur:
    ...:
    ...:     CountryCurrency.objects.create(**dict([("symbol", cc.get_symbol(cu)), ("name", cc.get_currency_name(cu))]))
cur = c.get_rates('USD')
'''
import yfinance as yf
